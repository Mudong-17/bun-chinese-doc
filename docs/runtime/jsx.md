...
Bun 会读取您的 `tsconfig.json` 或 `jsconfig.json` 配置文件，以确定如何在内部执行 JSX 转换。为了避免使用这两者之一，以下选项也可以在 [`bunfig.toml`](/docs/runtime/configuration) 中定义。
...
- ```tsx
  ...
  `jsxDEV` 变量名是 React 使用的约定。 `DEV` 后缀是一种可见的方式，表示该代码用于开发中使用。开发版本的 React 速度较慢，包括额外的有效性检查和调试工具。
